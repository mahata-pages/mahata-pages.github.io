...
function PostContent({ baseDir, slug }: PostProps & { slug: string }) {
  const posts = baseDir === "posts-test" ? testPosts : prodPosts;
  const postPath =
    baseDir === "posts-test" ? `../../posts-test/${slug}.md` : `../../posts/${slug}.md`;
  ...
  const renderedHtml = useMemo(() => {
    try {
      const processed = remark().use(frontmatter).use(html).processSync(markdown);
